...
from characters import spawnBarbarian, move_barbarians, spawnArcher, move_archers, spawnDragon, move_dragons, spawnBalloon, move_balloons, clearTroops,spawnStealthArcher,move_stealthArcher,spawnHealer,move_healers,barbarians,archers,dragons,balloons,stealthArcher,healers
...
while(True):
    ...
    if ch == 'w':
        ...
        spawnStealthArcher(V.spawn_points[0],cnt) #Passing current game time so that a timer can be set for invisibility of stealth archer
    elif ch == 'y':
        spawnStealthArcher(V.spawn_points[1],cnt)
    elif ch == 'u':
        spawnStealthArcher(V.spawn_points[2],cnt)
    elif ch == 'q':
        print('quit')
        break
    # os.system('clear')
    ...
    move_stealthArcher(V,pt.movement)
    move_dragons(V)
    move_balloons(V)
    move_healers(V)
    shoot_cannons(King, V,cnt)
    shoot_wizard_towers(King, V,cnt)
    if(cnt % 20 == 0):
        os.system('clear')
    print("\033[%d;%dH" % (0, 0), end='')
    printMap(V)
    ...
    if(V.check_if_game_over(King) == 1):  # next level
        ...
        print('Defeat')
        break
    # elif len(troops) == 0 and len(healers) > 0:
    #     print("Game Over - Only Healers Left!")
    #     break
