...
class Cannon(Building):
    ...
    def scan_for_targets(self, King, current_time):
        self.isShooting = False

        troops = barbarians + archers +stealthArcher

        for troop in troops:
            if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.attack_radius**2:
                self.isShooting = True
                self.attack_target(troop,current_time)
                return
...
class WizardTower(Building):
    ...
    def scan_for_targets(self, King,current_time):
        ...
        for troop in troops:
            if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.attack_radius**2:
                self.isShooting = True
                self.attack_target(troop,0,current_time)
                return
    ...
    def attack_target(self, target, isKing,current_time):
        ...
        troops = barbarians+ archers + dragons + balloons +stealthArcher+healers
...
def shoot_cannons(King, V,current_time):
    for cannon in V.cannon_objs:
        V.cannon_objs[cannon].scan_for_targets(King,current_time)


def shoot_wizard_towers(King, V,current_time):
    for tower in V.wizard_tower_objs:
        V.wizard_tower_objs[tower].scan_for_targets(King,current_time)
