...
import numpy as np
import datetime
from characters import barbarians, dragons, balloons, archers, stealthArcher,healers
...
def printMap(V):
    ...
    for stealtharch in stealthArcher:
        a= 2*stealtharch.position[0]
        b = 2*stealtharch.position[1]
        map_matrix[a][b] = stealtharchColor(stealtharch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b] = stealtharchColor(stealtharch) + Fore.BLACK + '  '
    ...
    for dr in dragons:
        with open("hello.txt", "w") as my_file:
            ...
            # my_file.write( i for i in dr.position  )
            # my_file.write (" is the dr position")
            my_file.write( str(type(dr.position)) )
            my_file.write( " is the type of dr position \n")
            my_file.write( str(dr.position[0]))
            my_file.write(  " is the dr position[0] \n"  )
            my_file.write( str(dr.position[0] ))
            my_file.write(  " is the type of dr position[0]\n\n")
    ...
    for hr in healers:
        with open("hello1.txt", "w") as my_file:
            my_file.write("Hello world \n")
            # my_file.write( i for i in hr.position  )
            # my_file.write (" is the hr position")
            my_file.write( str(type(hr.position)) )
            my_file.write( " is the type of hr position\n")
            my_file.write( str(hr.position[0]))
            my_file.write(  " is the hr position[0]\n"  )
            my_file.write( str(hr.position[0] ))
            my_file.write(  " is the type of hr position[0]\n\n")

        a= 2*hr.position[0]
        b = 2*hr.position[1]
        a = int( a )
        b = int( b )
        map_matrix[a][b] = hrColor(hr) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = hrColor(hr) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = hrColor(hr) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = hrColor(hr) + Fore.BLACK + '  '
